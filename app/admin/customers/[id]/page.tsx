...
export default function CustomerDetailPage({ params }: { params: { id: string } }) {
  ...
  if (isLoading) {
    return (
      <div className="p-8">
        <div className="h-6 w-32 bg-stone-200 rounded animate-pulse mb-8" />
        <div className="h-64 bg-white rounded-2xl animate-pulse mb-4" />
  ...
  if (!customer) {
    return (
      <div className="p-8 text-center text-stone-500">
        ...
        <button onClick={() => router.back()} className="mt-4 text-primary-600 text-sm hover:underline">Go back</button>
      </div>
    );
  }
  ...
  return (
    <div className="p-8">
      ...
      <button
        onClick={() => router.back()}
        className="flex items-center gap-1.5 text-sm text-stone-500 hover:text-espresso mb-6 transition-colors"
      >
      ...
      <div className="bg-white rounded-2xl shadow-soft border border-stone-200/70 p-6 mb-6">
        <div className="flex flex-wrap items-start justify-between gap-6">
          ...
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-primary-100 rounded-full flex items-center justify-center flex-shrink-0">
              <span className="text-2xl font-bold text-primary-600">{customer.name[0]}</span>
            </div>
            <div>
              <div className="flex items-center gap-3 flex-wrap">
                <h1 className="text-xl font-display font-bold text-espresso">{customer.name}</h1>
                {customer.customerId && (
                  <span className="font-mono text-xs font-semibold text-primary-700 bg-primary-50 border border-primary-100 px-2.5 py-1 rounded-full">
                    CID: {customer.customerId}
                  </span>
                )}
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                  customer.banned ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'
                }`}>
                  {customer.banned ? 'Banned' : 'Active'}
                </span>
              </div>
              <p className="text-sm text-stone-400 mt-0.5">Member since {formatDate(customer.createdAt)}</p>
          ...
          {/* Summary stats */}
          <div className="flex items-center gap-4">
            <div className="bg-stone-50 rounded-xl px-4 py-3 text-center min-w-[80px]">
              <p className="text-xs text-stone-500 mb-0.5">Orders</p>
              <p className="text-xl font-bold text-espresso">{customer.orders.length}</p>
            </div>
            <div className="bg-primary-50 rounded-xl px-4 py-3 text-center min-w-[100px]">
              <p className="text-xs text-primary-500 mb-0.5">Total Spent</p>
        ...
        {customer.banned && (
          <div className="flex items-center gap-2 p-3 bg-red-50 rounded-xl text-sm text-red-700 mt-4">
        ...
        <div className="mt-5 pt-5 border-t border-stone-100 space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="flex-1 text-sm border border-stone-200 rounded-xl px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-primary-400"
            />
          </div>

          {/* Address */}
          <div>
            <p className="flex items-center gap-1.5 text-sm text-stone-500 font-medium mb-2">
              <MapPin className="h-4 w-4 text-stone-400" /> Delivery Address
            </p>
          ...
          <div className="flex items-center gap-3 pt-1">
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                saved ? 'bg-emerald-100 text-emerald-700' : 'bg-espresso hover:bg-primary-800 text-cream'
              }`}
            >
      ...
      <div className="bg-white rounded-2xl shadow-soft border border-stone-200/70 overflow-hidden">
        <div className="flex items-center gap-2 px-6 py-4 border-b border-stone-100">
          ...
          <h2 className="font-display font-semibold text-espresso">Order History</h2>
          <span className="ml-1 bg-stone-100 text-stone-600 text-xs font-medium px-2 py-0.5 rounded-full">
        ...
        {customer.orders.length === 0 ? (
          <div className="px-6 py-12 text-center text-stone-400 text-sm">
            <ShoppingBag className="h-8 w-8 mx-auto mb-2 text-stone-200" />
            ...
            <table className="w-full text-sm">
              <thead className="bg-stone-50 border-b border-stone-200">
                <tr>
                  ...
                  <th className="text-left px-6 py-3 text-xs font-medium text-stone-500 uppercase tracking-wide">Date Ordered</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-stone-500 uppercase tracking-wide">Items</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-stone-500 uppercase tracking-wide">Status</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-stone-500 uppercase tracking-wide">Payment</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-stone-500 uppercase tracking-wide">Delivery</th>
                    ...
                    <tr key={order.id} className="hover:bg-stone-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-0.5">
                          {order.items.map(item => (
                            <p key={item.id} className="text-stone-700 text-xs">
                              {item.product.name}
                              <span className="text-stone-400 ml-1">×{item.quantity}</span>
                            </p>
                          ))}
                        </div>
                        <p className="text-xs text-stone-400 mt-1">{totalQty} item{totalQty !== 1 ? 's' : ''} total</p>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span className="flex items-center gap-1.5 text-stone-600 text-xs">
                          <CreditCard className="h-3.5 w-3.5 text-stone-400 flex-shrink-0" />
